...
function testInt(ctor, shift, sizes, scale, disp) {
...
function testFloat(ctor, shift, sizes, scale, disp) {
    for (var bytes of sizes) {
        var ab = new ArrayBuffer(bytes);
        var arr = new ctor(ab);
        for (var i = 0; i < arr.length; i++)
...
function test(tester, ctor, shift, sizes) {
    for (scale of [0,1,2,3]) {
        for (disp of [0,1,8,Math.pow(2,31)-1,Math.pow(2,31),Math.pow(2,32)-1])
            tester(ctor, shift, sizes, scale, disp);
    }
}

test(testInt, Int8Array, 0, [1, 256]);
test(testInt, Uint8Array, 0, [1, 256]);
test(testInt, Int16Array, 1, [2, 256]);
test(testInt, Uint16Array, 1, [2, 256]);
test(testInt, Int32Array, 2, [4, 256]);
test(testInt, Uint32Array, 2, [4, 256]);
test(testFloat, Float32Array, 2, [4, 256]);
test(testFloat, Float64Array, 3, [8, 256]);
